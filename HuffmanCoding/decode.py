...
def Huffman_Coding(picture):
    ...
    for key in coding_table.keys():
        print("信源像素点" + key + "编码后的码字为:" + coding_table[key])
    ...
    file = open('coding_resultLena.txt', 'w')
...
def Decoding(width, height, coding_table, coding_result):
    ...
    for i in range(width):
        for j in range(height):
            ...
            k += 1
    decode_image.save('result/decodenewLena.bmp')
    print("译码已经完成：图片存储为decodenewLena.bmp")

if __name__ == '__main__':
    picture=picture_convert('img/Lena.bmp','result/newLena.bmp')
